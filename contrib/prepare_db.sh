...
while true; do
	...
	echo "⏳ Waiting on DB... (${CUR_DB_WAIT_TIME}s / ${MAX_DB_WAIT_TIME}s), last error:"
...
if [ "$SKIP_SUPERUSER" = "true" ]; then
	...
	if [ -z ${SUPERUSER_PASSWORD+x} ] && [ -f "/run/secrets/superuser_password" ]; then
		echo "⚙️ Reading SUPERUSER_PASSWORD from Docker Secrets"
...
if User.objects.filter(username=superuser_name).exists():
    print(f'↩️ Superuser \033[96m{superuser_name}\033[0m already exists, skip creation')
    exit()

if User.objects.exists():
    print('↩️ Skip creating of superuser, as other users already exist')
    exit()

User.objects.create_superuser(superuser_name, superuser_email, superuser_password)
print(f'💡 Superuser \033[96m{superuser_name}\033[0m created')
END
fi
