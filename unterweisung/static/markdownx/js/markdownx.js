 ...
 */
function scanForEditors(root) {
    var elements = root.getElementsByClassName('markdownx');
    Object.keys(elements).forEach(function (key) {
        ...
        if (!editor.hasAttribute('data-markdownx-init') || true)
...
docReady(function () {
    ...
    document.addEventListener('formset:added', function (event) {
        if (event.target instanceof HTMLElement)
            scanForEditors(event.target);
    });
    // initialize
    scanForEditors(document.getElementsByTagName('html')[0]);
});
